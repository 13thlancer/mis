...
<script type="text/javascript">
                    ...
                    columns: [
                        ...
                        {data: 'picpath'},
                        {
                            "render": function (data, type, row) {
                                var id= '"' + row.id + '"';
                                var username = '"' + row.senderid + '"';
                                var recievername = '"' + row.reciverid + '"';
                                var sendcontent= '"' + row.sendcontent + '"';
                                var picpath= '"' + row.picpath + '"';
                                var sendtype= '"' + row.type + '"';
                                var html = "<a href='javascript:void(0);'  onclick='edit("+ id + ","+username+","+recievername+","+sendcontent+","+picpath+","+sendtype+")' class='up btn btn-default btn-xs'><i class='fa fa-arrow-up'></i> 编辑</a>"
                                html += "<a href='javascript:void(0);'   onclick='del("+ id + ")'  class='down btn btn-default btn-xs'><i class='fa fa-arrow-down'></i> 删除</a>"
                                return html;
                            }
                        }
                    ]
    ...
    function check(obj) {
     ...
     /*获取选中行信息*/
    function edit(id ,username,recievername,sendcontent,picpath,sendtype){
        $("#update").modal("show");
        document.getElementById("update_blogid" ).value=id;
        document.getElementById("update_username").value=username;
        document.getElementById("update_recievername").value=recievername;
        document.getElementById("update_sendcontent").value=sendcontent;
    ...
    function updateblog(){
      var postData = {
          ...
          picpath:document.getElementById("update_picpath").value,
          sendtype:document.getElementById("update_sendtype").value
      };
      $.ajax({
          type:"POST",
